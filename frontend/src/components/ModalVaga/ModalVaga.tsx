...
interface ModalVagaProps {
    isOpen: boolean;
    onClose: () => void;
    onSuccess: (vaga: VagaData) => void;
    vagaParaEditar?: VagaData | null;
}
...
export const ModalVaga = ({ isOpen, onClose, onSuccess, vagaParaEditar }: ModalVagaProps) => {
    ...
    const handleChipClick = (valor: string) => {
        ...
        if (valor === 'todos') {
            atualizados = formData.tiposUsuario.includes('todos') ? [] : ['todos'];
        } else {
            const semTodos = formData.tiposUsuario.filter(t => t !== 'todos');
            atualizados = semTodos.includes(valor)
                ? semTodos.filter(t => t !== valor)
                : [...semTodos, valor];
        }
        setFormData(prev => ({ ...prev, tiposUsuario: atualizados }));
        if (errors.tiposUsuario) setErrors(prev => ({ ...prev, tiposUsuarios: '' }));
    };
    ...
    const validar = () => {
        ...
        else if (formData.data < hoje) newErrors.data = 'Data no passada não é permitida';

        if (formData.descricao.length < 10) newErrors.descricao = 'Mínimo 10 caracteres';
        if (formData.requisitos.length < 5) newErrors.requisitos = 'Mínimo 5 caracteres';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const vagaSalva = await res.json();
            onSuccess(vagaSalva);
            onClose();
        } catch (error) {
            console.error(error);
            ...
            <div className={styles.modalContent}>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
                    <div className={styles.modalBody}>
                        <div>
                            ...
                            <div className={styles.grid}>
                                ...
                                <div className={styles.campo}>
                                    ...
                                    <input name="modalidade" value={formData.modalidade} onChange={handleChange} className={errors.modalidade ? styles.inputErro : ''} placeholder="Presencial, Remoto, Hibrido" />
                                ...
                                <div className={styles.campo}>
                                    <label>Salário</label>
                                    <input name="salario" value={formData.salario} onChange={handleChange} className={errors.salario ? styles.inputErro : ''} placeholder="Ex: 1.500,00" />
                        ...
                        <div className={styles.secaoTitulo}>Quem pode se candidatar</div>
                            <div className={styles.tiposGrid}>
                                ...
                                >
                                    <i className="fas fa-briefcase"></i> Prestador de Serviço
                                </button>
                    ...
                    <div className={styles.modalFooter}>
                        <button type="button" className="btn-secundary" onClick={onClose}>Cancelar</button>
                        <Button type="submit" isLoading={isSubmitting}>Salvar Vaga</Button>
                    </div>
                </form>
            </div>
        </div>
