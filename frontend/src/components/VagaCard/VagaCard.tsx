...
export const VagaCard: React.FC<VagaCardProps> = ({
  ...
  const isDono = Boolean(usuarioLogado && vaga.usuarioId === usuarioLogado.id);
  ...
  const euCurtiu = Boolean(usuarioLogado && vaga.likes?.includes(usuarioLogado.id));
  const euDescurtiu = Boolean(usuarioLogado && vaga.dislikes?.includes(usuarioLogado.id));
  ...
  return (
    <div className={styles.vagaCard} data-vaga-id={vaga.id}>
      <div className={styles.header}>
        <img className={styles.avatar} src={FOTO_DEFAULT} alt="Foto do autor" />
        <div className={styles.autor}>
          <div className={styles.autorNome}>{vaga.nomeUsuario || 'Usuário'}</div>
          <div className={styles.autorMeta}>
            {vaga.empresa} {vaga.data ? `· ${vaga.data}` : ''}
          </div>
        </div>
      ...
      <div className={styles.body}>
        ...
        <div className={styles.detalhes}>
          <div className={styles.detalheBloco}>
            ...
            <div className={styles.detalheValor}>{vaga.beneficios || '—'}</div>
          </div>
          <div className={styles.detalheBloco}>
            <div className={styles.detalheLabel}>Requisitos</div>
            <div className={styles.detalheValor}>{vaga.requisitos || '—'}</div>
          </div>
        </div>
      ...
      <div className={styles.acoes}>
        ...
        >
          <ThumbsUp size={18} fill={euCurtiu ? "currentColor" : "none"} />
          <span>{nLikes}</span>
        </button>

        <button
          className={`${styles.acaoBtn} ${euDescurtiu ? styles.disliked : ''}`}
          title="Não curtir"
          onClick={() => onDislike(vaga.id)}
        >
          <ThumbsDown size={18} fill={euDescurtiu ? "currentColor" : "none"} />
          <span>{nDislikes}</span>
        </button>
        ...
        {isDono ? (
          <span className={`${styles.btnCandidatar} ${styles.candidatarDono}`}>
            Sua vaga
          </span>
        ) : podeCand ? (
          <a
          ...
          <span className={`${styles.btnCandidatar} ${styles.candidatarBloqueado}`}>
            Fora do perfil
          </span>
        )}
      </div>
